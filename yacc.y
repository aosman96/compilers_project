...
code : code line 
 ...
 line   : exp SEMICOLON   {printf (" decleration \n");}
	...
	| SCOPE_BRACEOPEN line SCOPE_BRACECLOSE {printf("a scope decleration \n");}
	| KEYWORD_WHILE ARGUMENT_OPENBRACKET conditional_statment ARGUMENT_CLOSEBRACKET Inside_loops_statments {printf(" a while loop \n");}  // the while loop
 ...
 exp    :                    
	...
	| exp OPERATOR_DIVIDE exp                    {printf("division statement");}
	| exp OPERATOR_PLUS exp                        {printf("plus statement");} // + 
        | exp OPERATOR_MINUS exp			{printf("minus statement");}
	| ARGUMENT_OPENBRACKET exp ARGUMENT_CLOSEBRACKET {printf(" argument experation \n");}
	| value 
        ;
...
case_statment : 
		KEYWORD_CASE value COLON line KEYWORD_BREAK SEMICOLON case_statment   {printf("CAse statemnent");}
		| 
		;
...
conditional_statment :
		       exp
		     | exp RELATION_GREATER exp {printf(" greater than statement \n");}
		     | exp RELATION_LESSER exp {printf(" lesser than statement \n");}
		     | exp RELATION_EQUALS exp  {printf(" equal statement \n");}
		     | exp RELATION_NOTEQUAL exp  {printf(" not equal statement \n");}
		     | exp RELATION_LESSEREQUAL exp  {printf(" less equal than statement \n");}
		     | exp RELATION_GREATEREQUAL exp {printf(" greater equal than statement \n");}
		     |  conditional_statment LOGIC_AND conditional_statment {printf(" AND statement \n");}
		     |	 conditional_statment LOGIC_OR conditional_statment {printf(" OR statement \n");}
		     |  LOGIC_NOT conditional_statment  {printf(" LOGICAL NOT statement \n");}
